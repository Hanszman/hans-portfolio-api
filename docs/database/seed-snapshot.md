...
The current snapshot already includes the complete Spanish content. Its
language-specific backfill was intentionally removed after the rollout because
it is not part of normal seed or runtime maintenance. Any future language must
follow the same non-destructive sequence: export the pre-change snapshot, add
nullable columns, create and validate a temporary idempotent backfill, apply
required constraints, export the new snapshot, verify the reseed in a disposable
database, and then remove the one-time backfill. Never use `prisma:seed` or reset
against the source database as a substitute for an additive migration and
audited backfill.
